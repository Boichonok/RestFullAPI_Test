...
public class SecurityInterceptor implements PreProcessInterceptor{
	...
	private boolean isUserAllowed(final String username, final String password, final Set<String> rolesSet) {
		...
		//System.out.println("Account: " + acc.getUserName() + " | " + "pass: " + acc.getPassword() + " | role: " + acc.getRole()[0] + acc.getRole()[1]);
		int countEqualsRole = 0;
		System.out.println("rolesSet size: " + rolesSet.size());
		if(rolesSet.size() == acc.getRole().length) {
			...
			System.out.println("Count Equals Role: " + countEqualsRole);
			if (countEqualsRole == acc.getRole().length) {
				return true;
			}
		}
		return false;
